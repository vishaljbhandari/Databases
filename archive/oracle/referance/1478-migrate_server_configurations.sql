...
INSERT INTO FRAUD_TYPES (ID, NAME, FRAUD_TYPE, DESCRIPTION, IS_ACTIVE)
	SELECT FRAUD_TYPES_SEQ.nextval, NAME, TYPE, DESCRIPTION, 1 FROM FRAUD_TYPE@DU_RANGER ;

INSERT INTO CASE_REASONS (ID, REASON, DESCRIPTION)
	SELECT CASE_REASONS_SEQ.nextval, REASON, DESCRIPTION FROM CASE_REASON@DU_RANGER WHERE REASON != 'Default' ;

INSERT INTO DISTANCE_TIMES (ID, LOWER_LIMIT, UPPER_LIMIT, TIME)
	SELECT DISTANCE_TIMES_SEQ.nextval, LOWER_LIMIT, UPPER_LIMIT, TIME FROM DISTANCE_TIME@DU_RANGER ;
INSERT INTO DISTANCE_TIMES_NETWORKS (NETWORK_ID, DISTANCE_TIME_ID) SELECT 999, ID FROM DISTANCE_TIMES ;

INSERT INTO PREPAID_TOP_UPS (ID, RATE_PLAN, MIN_AMOUNT, MAX_AMOUNT, DESCRIPTION)
	SELECT PREPAID_TOP_UPS_SEQ.nextval, RATE_PLAN, LOWER_LIMIT, UPPER_LIMIT, DESCRIPTION FROM PREPAID_TOP_UPS@DU_RANGER ;
...
DECLARE
	...
	IMSI VARCHAR(10) ;
BEGIN
	SELECT VALUE INTO CONF_VAL FROM CONFIGURATION@DU_RANGER WHERE ID = 'MoveToBlacklistActions' ;
...
UPDATE FIELD_CONFIGS SET DS_CATEGORY = TRIM(REPLACE(DS_CATEGORY,'SMM.PRECHECK'))
	WHERE RECORD_CONFIG_ID IN (3,4) AND DS_CATEGORY LIKE 'SMM.PRECHECK%'
		AND DATABASE_FIELD NOT IN (
			SELECT DATABASE_FIELD FROM DS_FIELD_CONFIG@DU_RANGER WHERE DS_TYPE IN (2,5) AND DS_CATEGORY LIKE '%SMM.PRECHECK%'
		) ;
