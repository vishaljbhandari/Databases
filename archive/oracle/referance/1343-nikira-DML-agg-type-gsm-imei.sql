spool nikira-DML-agg-type-gsm-imei.log
set feedback off ;
...
BEGIN
        ...
        then
            update aggregation_types set RECORD_CONFIG_IDS = RECORD_CONFIG_IDS || '1' where description = 'IMEI NUMBER' and id = 3;
        else
            update aggregation_types set RECORD_CONFIG_IDS = RECORD_CONFIG_IDS || ',1' where description = 'IMEI NUMBER' and id = 3;
        end if ;
		commit ;
...
insert into counter_manager_mappings (counter_detail_id, counter_type_id, aggregation_type_id, record_config_id) values (1, 13, 3, 1) ;
