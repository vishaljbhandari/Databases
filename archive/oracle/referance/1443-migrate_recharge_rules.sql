create or replace package MigrateRechargeRules
as
...
end MigrateRechargeRules ;
/
show error;

create or replace package body MigrateRechargeRules
as
	procedure MigrateThresholds(in_event_instance_id IN NUMBER, in_event_version IN NUMBER, in_event_code IN NUMBER, in_rule_key IN NUMBER) 
	IS
		...
		sub_id NUMBER(20) ;
		th_value NUMBER(16,6) ;
		th_duration NUMBER(20) ;
	begin
	...
	begin
		...
		exception when no_data_found then
			UTL_FILE.PUTF (fHandler, 'In GetFieldConfigDetails in_filter_field - ' || in_filter_field || ' in_rec_conf_id - ' || in_rec_conf_id ||'\n') ;
		end ;
	...
	begin
		LOOP
			...
			GetFieldConfigDetails(filter_row.filter_field, 2,  filter_field_id, filter_data_type, filter_field_name, translation_id) ;
			...
			then
				...
				then
					...
					then
					   BEGIN
							...
							strDescription := 'Recharge Log.' || filter_field_name || ' ' || filter_operator_name || ' ' || filter_row.STRING_VALUE ;
						EXCEPTION
				...
				then
					strDescription := 'Recharge Log.' || filter_field_name ||' '|| filter_operator_name ||' '|| translated_string ; 
				end if ;
			end if ;
	...
	BEGIN
		...
		--DataStream : Recharge Log
		insert into rules_tags(RULE_ID, TAG_ID) values (current_rule_id, 15) ;
	...
	IS
		...
		select * from EVENT_INSTANCE@DU_NIKIRA where code in (57, 58, 59, 60)
	...
	BEGIN
		fHandler := UTL_FILE.FOPEN(log_file_location, 'migrate_recharge_rules.log', 'w') ;
		...
		loop
			...
			--record config : Recharge Log
			insert into record_configs_rules (rule_id, record_config_id) values (current_rule_id, 2) ;
			...
			--Migrating filters
			select count(1) into filter_count from filter@DU_NIKIRA where EVENT_INSTANCE_ID = event_row.id and 
					EVENT_VERSION = event_row.event_version and is_exception = 0 ;
			if (filter_id = 0 and filter_count > 0)
			then
				insert into filters(ID, NAME, RECORD_CONFIG_ID, IS_EXCEPTION, NETWORK_ID, USER_ID, IS_ACTIVE, CATEGORY) values
					(filters_seq.nextval, null, 2, 0, 999, null, 1, null) ;
				insert into filters_rules(filter_id, rule_id) values (filters_seq.currval, current_rule_id) ;
			...
			begin
				if(event_row.code = 58)
				then
					insert into expressions (ID, LEFT_FIELD, RIGHT_FIELD, RIGHT_FIELD_INDICATOR, OPERATOR_ID, PRIORITY) values
						(expressions_seq.nextval, '5', '1,3', 1030, 8, 0) ;
					insert into filter_tokens(ID, FILTER_ID, DESCRIPTION, EXPRESSION_ID, CONTAINED_FILTER_ID, SYSTEM_FILTER, BEGIN_GROUP_IND,
						COMBINING_OPERATOR, END_GROUP_IND) values
						(filter_tokens_seq.nextval, filter_id, 'Recharge Log.Status In Success,Reversal', expressions_seq.currval, null, 1, null, null, null) ;
				end if ;

				if(event_row.code = 60)
				then
					insert into expressions (ID, LEFT_FIELD, RIGHT_FIELD, RIGHT_FIELD_INDICATOR, OPERATOR_ID, PRIORITY) values
						(expressions_seq.nextval, '5', '2', 1024, 6, 0) ;
					insert into filter_tokens(ID, FILTER_ID, DESCRIPTION, EXPRESSION_ID, CONTAINED_FILTER_ID, SYSTEM_FILTER, BEGIN_GROUP_IND,
						COMBINING_OPERATOR, END_GROUP_IND) values
						(filter_tokens_seq.nextval, filter_id, 'Recharge Log.Status Equal Failure', expressions_seq.currval, null, 1, null, null, null) ;
				end if ;
			exception when others then
				UTL_FILE.PUTF (fHandler, 'Insertion failed for system filters\n') ; 
			end ;
			...
			then
				insert into filters(ID, NAME, RECORD_CONFIG_ID, IS_EXCEPTION, NETWORK_ID, USER_ID, IS_ACTIVE, CATEGORY) values
					(filters_seq.nextval, null, 2, 1, 999, null, 1, null) ;
				insert into filters_rules(filter_id, rule_id) values (filters_seq.currval, current_rule_id) ;
				select filters_seq.currval into filter_id from dual ;
		...
		<<exit_on_error>>
		UTL_FILE.PUTF (fHandler, 'MigrateRechargeRules completed\n') ;
