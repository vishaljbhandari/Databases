...
insert into rc_record_configs (select rc_record_configs_seq.nextval, 1, id, 'RULE' from tasks where name = 'CDR Rule Creation') ;
insert into rc_record_configs (select rc_record_configs_seq.nextval, 1, id, 'TEMPLATE' from tasks where name = 'CDR Template Creation') ;
