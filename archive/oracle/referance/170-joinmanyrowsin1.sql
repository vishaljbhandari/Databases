...
CREATE OR REPLACE TYPE BODY JoinManyRowsIn1Impl IS
      ...
      begin
	if self.string is null
	then
		self.string := ctx2.string ;
	elsif ctx2.string is not null
	then
		self.string := self.string || delimiter || ctx2.string ;
	end if ;
        return ODCIConst.Success;
      end;
...
CREATE TYPE JoinManyRowsIn1ImplUniq AS OBJECT
(
      string varchar2(32767),
      delimiter varchar2(32767),
	  isAggregateMergeCalled number(1),
...
CREATE OR REPLACE TYPE BODY JoinManyRowsIn1ImplUniq IS
      ...
      return number is
      begin
        sctx := JoinManyRowsIn1ImplUniq('',',',0);
        return ODCIConst.Success;
      end;
      ...
      begin
		if self.isAggregateMergeCalled = 1 and self.string is not null
		then
		   tempString := '' ;
		   for record in (select distinct regexp_substr(self.string,'[^,]+', 1, level) as field from dual connect by regexp_substr(self.string, '[^,]+', 1, level) is not null)
		   loop
				if tempString is not null
				then
					tempString := tempString || delimiter ;
				end if ;
				tempString := tempString || record.field ;
		   end loop ;
...
CREATE FUNCTION JoinManyRowsIn1Uniq (input varchar2) RETURN varchar2
...
CREATE TYPE JoinManyRowsIn1WithAndImpl AS OBJECT
(
     string varchar2(32767),
     delimiter varchar2(32767),
...
CREATE FUNCTION JoinManyRowsIn1WithAnd (input varchar2) RETURN varchar2
