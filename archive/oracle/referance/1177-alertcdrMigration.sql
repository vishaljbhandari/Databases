...
cursor cur is select ac.cdr_id, a.aggregation_value, a.aggregation_type, a.start_day,a.end_day, ac.RECORD_CATEGORY, a.id
			from alerts a, alert_cdr ac where a.ID=ac.alert_id  and ac.RECORD_CATEGORY not in (select RECORD_CONFIG_ID from NO_DENORMALIZATION_TYPES ) order by start_day, RECORD_CATEGORY ;
...
begin
		...
		loop
			...
			loop
					...
					begin
						select LOOKUP_TABLE_NAME into InsertTableName from alarm_xdr_maps where record_config_id = RecordCatagories(i) ;
						EXCEPTION 
						WHEN OTHERS THEN
						GOTO START_LOOP ;
					end ;
					...
					if (GotData = true)
					then
						update alert_cdr set RUN_ID=ALERT_CDR_RUN_ID_SEQ.nextval, AGGREGATION_TYPE=AgTypes(i), AGGREGATION_VALUE=AgValues(i), TIME_STAMP=XDR_Time
...
/
	ALTER table alert_cdr add RUN_ID NUMBER(20) NOT NULL ; 
	ALTER table alert_cdr add AGGREGATION_TYPE NUMBER(20) NOT NULL ;
	ALTER table alert_cdr add AGGREGATION_VALUE VARCHAR2(40) ;
	ALTER table alert_cdr add TIME_STAMP DATE NOT NULL ;
	ALTER table alert_cdr add IS_CALLED_TO_CALLED_BY number(1) default 0 NOT NULL ;
