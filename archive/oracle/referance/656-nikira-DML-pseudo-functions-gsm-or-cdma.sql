...
DECLARE
	...
	BEGIN
		...
		if (is_gsm_selected = 1 or is_cdma_selected = 1) then
			...
			insert into pseudo_functions (id, name, category, accumulation_function_id, accumulation_field_id, addtnl_accumulation_field_id) Values (15, 'Call Collision', null, 7, 12, 3) ;
			insert into pseudo_functions (id, name, category, accumulation_function_id, accumulation_field_id, addtnl_accumulation_field_id) Values (105, 'Call Collision Phone Number', null, 7, 12, 2) ;
		end if ;
END ;
/		
