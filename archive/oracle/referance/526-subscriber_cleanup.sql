...
AS
	...
	BEGIN
		...
		for FpTableName in (select tname from fp_profiles_dataset_map where entity_id = FP_SUBSCRIBER_ENTITY and run_identifier <> -1)
		loop
			execute immediate 'DELETE FROM ' || FpTableName.tname || ' WHERE entity_id = ' || FP_SUBSCRIBER_ENTITY || ' AND profiled_entity_id = ' || sub_id || ' AND version = 0 ';
		end loop;
