spool nikira-DML-counter-map-entries-gsm.log
set feedback off ;
...
--Distinct Count
insert into counter_maps(id, aggregation_type, record_config_id, function_id, counter_type)
                        (select counter_maps_seq.nextval, 2,record_configs.id, functions.id, 1
                            from functions,record_configs
                             where functions.id in (55 )
                             and record_configs.tname = 'CDR');

insert into counter_maps(id, aggregation_type, record_config_id, function_id, counter_type)
                        (select counter_maps_seq.nextval, 7,record_configs.id, functions.id, 1
                            from functions,record_configs
                             where functions.id in (55 )
                             and record_configs.tname = 'CDR');
commit ;
