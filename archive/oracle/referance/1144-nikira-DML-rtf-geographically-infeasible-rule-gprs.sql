...
spool nikira-DML-rtf-geographically-infeasible-rule-gprs.log
set feedback off ;
set serveroutput off ;

---Geographically Infeasible - GPRS Rule - For RTF -----------------------------
DECLARE
	...
	BEGIN
		...
		if (is_gsm_selected = 1 or is_cdma_selected = 1) then
				INSERT INTO rules(ID,PARENT_ID,KEY,NAME,VERSION,AGGREGATION_TYPE,ACCUMULATION_FIELD,ACCUMULATION_FUNCTION,PSEUDO_FUNCTION_ID,IS_ENABLED,CREATED_TIME,MODIFICATION_TIME,SEVERITY,USER_ID,DESCRIPTION,PROCESSOR_TYPE,CATEGORY,IS_ACTIVE,RULE_MODE) VALUES (rules_seq.nextval,'999',rules_key_seq.nextval,'RTF Geographically Infeasible - GPRS','0','2','45','4','9','0',TO_DATE('2007-01-04 11:07:39','YYYY-MM-DD HH24:MI:SS'),TO_DATE('2007-01-04 11:07:39','YYYY-MM-DD HH24:MI:SS'),'100','system',NULL,'1',null,'1',3) ;
				
				insert into rtf_test_runs values (RTF_TEST_RUNS_SEQ.nextval, 9, rules_seq.currval, 1, 3, 1, sysdate, sysdate+7, 0, 3, null, null, null);
				insert into rtf_test_run_results values (RTF_TEST_RUNS_SEQ.currval, sysdate, sysdate+7, 0, 0, 0, 0, 0, 0);
				...
				INSERT INTO thresholds(id, threshold_id, aggregation_type, rule_key, rule_version, value, version, duration, effective_time, effective_day, effective_week_day, changed_status) VALUES (thresholds_seq.nextval, 0, 2, rules_key_seq.currval, 0, 0, 0, 86400, 1, 1, 0, 0) ;
				
				insert into record_configs_rules (rule_id, record_config_id) values (rules_seq.currval, 7) ;
				insert into actions_rules (rule_id, action_id) values (rules_seq.currval, 61) ;
            	...
            	insert into rules_subscriber_groups (rule_id, subscriber_group_id)
                	                                            (select rules_seq.currval, s.id from subscriber_groups s
                    	                                         where
                        	                                     s.category = 1 and
                            	                                 s.record_config_id = 3);
				...
				insert into rules_tags(rule_id, tag_id) values(rules_seq.currval, (select id from tags where name='GPRS' and category='DATA_STREAMS'));
				insert into rules_tags(rule_id, tag_id) values(rules_seq.currval, (select id from tags where name='Velocity' and category='FUNCTIONS'));
