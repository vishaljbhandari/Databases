...
CREATE OR REPLACE FORCE VIEW "DSM_RECORD_CONFIGS_V" ("ID", "DESCRIPTION", "TNAME", "VISIBLE", "CATEGORY", "OFFLINE_TNAME", "SPARK_SUPPORT", "DISTRIBUTED_CACHE", "TASKS", "AGGREGATIONS", "STATS_AGGREGATIONS", "DENORMALIZE_TNAME", "DISPATCHER_CATEGORY", "PREVEA_SUPPORT", "DISPATCHER_ACTIONS", "STORAGE_DAYS", "DELIMETER", "FUNCTIONS", "ARCHIVE_TABLE_NAME", "ARCHIVE_TNAME", "STATUS") AS
  (
    ...
    from record_configs r where (description in (select name from tasks where parent_id = getparentid('Record View')) or id in (select distinct RECORD_CONFIG_ID from reference_types where RECORD_CONFIG_ID is not null) and delimeter is not null) and r.id not in (select id from record_configs_dsm where delete_record = 0)
union
...
CREATE OR REPLACE FORCE VIEW "DSM_NETWORK_V" ("ID", "SHORT_NAME", "DESCRIPTION", "CURRENCY") AS
  ...
  (
	    ...
	    (select joinmanyrowsin1(name) from dsm_field_categories_v where id in  (select field_category_id from field_record_config_maps where record_config_id = f.record_config_id and field_association = to_char(f.field_id) and field_category_id not in (select id from field_categories where name in (select FIELD_CATEGORY from field_configs_dsm where record_config_id = f.record_config_id and FIELD_CATEGORY is not null)))) as field_category,
...
select r.id, t.name as task_name from tasks t, record_configs r , record_view_configs rv where rv.record_config_id = r.id and (link like '%'||rv.id  and parent_id = getparentid(r.description, 'Record View'))
...
union select r.id, 'Record View' from tasks t, record_configs r , record_view_configs rv where rv.record_config_id = r.id and link like '/record_view/list/'||rv.id||'%' and parent_id = getparentid('Record View')
) group by id ;
