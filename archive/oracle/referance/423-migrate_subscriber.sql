...
DECLARE
	...
	CURR_ID NUMBER := 2024 ;
BEGIN
	SELECT MAX(ID) INTO MAX_ID FROM ACCOUNT@DU_RANGER ;
	...
	LOOP
		...
		FROM ACCOUNT@DU_RANGER A, BLACKLIST_ACCOUNT@DU_RANGER B
...
FROM BLACKLIST_ACCOUNT@DU_RANGER ;
...
DECLARE
	...
	CURR_ID NUMBER := 2024 ;
BEGIN
	SELECT MAX(ACCOUNT_ID) INTO MAX_ID FROM ACCOUNT_CREDIT_DETAIL@DU_RANGER ;
	...
	LOOP
		...
		FROM ACCOUNT_CREDIT_DETAIL@DU_RANGER AC, ACCOUNT A, BL_ACCOUNT_CREDIT_DETAIL@DU_RANGER B
...
SELECT
	...
	AC.PREV_INTERIM_LAST_PAY_DATE, AC.INTERIM_LAST_PAID_AMOUNT, AC.PREV_INTERIM_LAST_PAID_AMOUNT, AC.INTERIM_NO_OF_SLIPPAGES,
	AC.PREV_INTERIM_NO_OF_SLIPPAGES, AC.INTERIM_NO_OF_PAYMENTS, AC.PREV_INTERIM_NO_OF_PAYMENTS
FROM BL_ACCOUNT_CREDIT_DETAIL@DU_RANGER AC, ACCOUNT A
...
DECLARE
	...
	CURR_ID NUMBER := 2024 ;
BEGIN
	SELECT MAX(ACCOUNT_ID) INTO MAX_ID FROM ACCOUNT_RECHARGE_DETAIL@DU_RANGER ;
	...
	LOOP
		...
		FROM ACCOUNT_RECHARGE_DETAIL@DU_RANGER AR, ACCOUNT A, BL_ACCOUNT_RECHARGE_DETAIL@DU_RANGER B
...
SELECT
	...
	AR.GRACE_PERIOD_EXPIRY_DATE, AR.PREV_GRACE_PERIOD_EXPIRY_DATE, AR.CURRENT_BALANCE,
	AR.PREVIOUS_BALANCE, AR.LAST_RECHARGE_DATE, AR.LAST_CDR_TIME, AR.DIFF_BALANCE_PERCENTAGE
FROM BL_ACCOUNT_RECHARGE_DETAIL@DU_RANGER AR, ACCOUNT A
...
DECLARE
	...
	CURR_ID NUMBER := 2024 ;
BEGIN
	SELECT MAX(ID) INTO MAX_ID FROM SUBSCRIBER@DU_RANGER ;
	...
	LOOP
		...
		FROM SUBSCRIBER@DU_RANGER S, ACCOUNT A
		WHERE S.ACCOUNT_ID = A.ID AND S.ID BETWEEN START_ID AND CURR_ID AND S.STATUS != 0 ;
...
FROM BLACKLIST_SUBSCRIBER@DU_RANGER S, ACCOUNT A
...
DECLARE
	SEQ_VALUE NUMBER ;
BEGIN
	SELECT SUBSCRIBER_ID.NEXTVAL@DU_RANGER INTO SEQ_VALUE FROM DUAL ;
