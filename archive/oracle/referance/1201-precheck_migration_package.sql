...
SET SERVEROUTPUT ON ;
SET FEEDBACK ON ;
create or replace package MigratePrecheckMatchedRecords
as
	PROCEDURE Migrate  ;
	...
	ret_val						pls_integer	 ;	
	current_matched_detail_id  	pls_integer  ;
	current_matched_record		pls_integer  ;
	is_insertion_required		pls_integer  ;

end MigratePrecheckMatchedRecords ;
/
show err ;
create or replace package body MigratePrecheckMatchedRecords
as
	...
	begin
		...
		FOR current_record in 
				(
				 select id, matched_results_id, matched_record, field_id, field_value, match_type
				 from 
				 matched_details
		...
		LOOP
				...
				then
					...
					current_matched_detail_id := current_record.id ;
				...
				then
					...
					insert into	mig_matched_details (id, matched_results_id, matched_record, field_ids, field_values, match_types) 
						values (current_matched_detail_id, current_matched_result_id, current_matched_record, field_ids_str,field_values_str, match_types_str) ;
					field_ids_str :=  current_record.field_id ;
					field_values_str := current_record.field_value ;
					match_types_str := current_record.match_type ;
					current_matched_detail_id := current_record.id ;
