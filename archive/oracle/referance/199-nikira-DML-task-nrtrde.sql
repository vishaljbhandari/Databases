...
BEGIN
	...
	insert into tasks (ID, PACKAGE_IDS, PARENT_ID, NAME, LINK, IS_DEFAULT, IS_MENU_ITEM ) (select tasks_seq.nextval, '0,4', parentid, name, '/vpmn_info/list/' || id ||'',0 , 1 from record_view_configs where is_visible = 1 and id = 49) ;
...
insert into dac_record_views_tasks (select dac_record_views_tasks_seq.nextval, 49, id from tasks where link like '%record_view/list/49%') ;
