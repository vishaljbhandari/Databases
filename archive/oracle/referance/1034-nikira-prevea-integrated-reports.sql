...
delete from translations where tr_id=900;
delete from translation_indices where id=900;

insert into translation_indices values (900, 'Exposure Type', '',0) ;
insert into translations values (translations_seq.nextval, 900,'EXPOSURE','Exposure');
insert into translations values (translations_seq.nextval, 900,'PERCENTAGE_EXPOSURE','Exposure Percentage');
...
insert into report_parameters (id, report_id, position, name, report_parameter_name, report_data_type_id,translation_index_id,is_mandatory)
	values (report_parameters_seq.nextval,reports_seq.currval, 2, 'Exposure Type', 'EXPOSURE_TYPE', 3,900,1);
...
insert into tasks (ID, PACKAGE_IDS, PARENT_ID, NAME, LINK, IS_DEFAULT, IS_MENU_ITEM ) values (tasks_seq.nextval, '0,1,2,3,4', GetParentID('Report List', 'Reports'), 'Exposure Management Report' , '', 0, 0) ;
