create or replace package MigrateArchivedAlarms
as
...
end MigrateArchivedAlarms ;
/
show error;

create or replace package body MigrateArchivedAlarms
as
	  FUNCTION GetRecordCategory(in_event_instance_id NUMBER) RETURN NUMBER
      IS
		event_code NUMBER ;
	...
	exception when no_data_found then
		...
		IS
				cursor cur_matched_subscriber(in_sub_id in NUMBER, in_ph_num in VARCHAR2) IS 
								select * from ARCHIVED_MATCHED_SUBSCRIBER@DU_NIKIRA where subscriber_id = in_sub_id and sub_phone_number = in_ph_num ;

				cursor cur_matched_field(in_matched_sub_id in NUMBER) IS select * from ARCHIVED_MATCHED_FIELD@DU_NIKIRA where ID = in_matched_sub_id ;

				cursor cur_active_details_cursor(in_sub_id in NUMBER, in_ph_num in VARCHAR2) IS
					select matched_field, MF.match_type, field_position, field_value from ARCHIVED_MATCHED_SUBSCRIBER@DU_NIKIRA MS, ARCHIVED_MATCHED_FIELD@DU_NIKIRA MF,
						ARCHIVED_PRE_SUBS_FIELD_INFO@DU_NIKIRA PS where MS.subscriber_id = in_sub_id
		...
		BEGIN
				...
				insert into archived_matched_results(ID, rule_key, AGGREGATION_VALUE, MATCH_PERCENTAGE, DB_TYPE)
					values (matched_results_seq.nextval, precheck_rule_key, in_phone_number, 0, 1) ;
				insert into AR_ALERT_MATCHED_RESULTS_MAPS(ALERT_ID, MATCHED_RESULTS_ID) values (in_alert_id, matched_results_seq.currval) ;

				insert into ARCHIVED_MATCHED_DETAILS(MATCHED_RESULTS_ID, MATCHED_RECORD, FIELD_ID, FIELD_VALUE, MATCH_TYPE)
					select matched_results_seq.currval, 0, FIELD_ASSOCIATION, '', null from PRECHECK_FIELD_CONFIGS ;

				select matched_results_seq.currval into matched_results_seq_id from dual ;

				update ARCHIVED_MATCHED_DETAILS set field_value = in_phone_number where MATCHED_RESULTS_ID = matched_results_seq_id
					and FIELD_ID = '12' ;
				update ARCHIVED_MATCHED_DETAILS set field_value = in_subscriber_id where MATCHED_RESULTS_ID = matched_results_seq_id
					and FIELD_ID = '32' ;
				update ARCHIVED_MATCHED_DETAILS set field_value = (select account_name from subscriber where
				...
				LOOP
					...
					update ARCHIVED_MATCHED_DETAILS set field_value = current_record.field_value where MATCHED_RESULTS_ID = matched_results_seq_id
				...
				LOOP
					...
					insert into ARCHIVED_MATCHED_DETAILS(MATCHED_RESULTS_ID, MATCHED_RECORD, FIELD_ID, FIELD_VALUE, MATCH_TYPE)
						select matched_results_seq.currval, 1, FIELD_ASSOCIATION, '', null from PRECHECK_FIELD_CONFIGS ;

					update ARCHIVED_MATCHED_DETAILS set field_value = matched_sub_row.matched_subscriber_id where 
						MATCHED_RESULTS_ID = matched_results_seq_id and FIELD_ID = '32' ;

					if(SQL%ROWCOUNT = 0)
					then
						insert into ARCHIVED_MATCHED_DETAILS(MATCHED_RESULTS_ID, MATCHED_RECORD, FIELD_ID, FIELD_VALUE, MATCH_TYPE)
							values(matched_results_seq.currval, 1, 32, matched_sub_row.matched_subscriber_id, null) ;
					end  if ;
					...
					then
						insert into ARCHIVED_MATCHED_DETAILS(MATCHED_RESULTS_ID, MATCHED_RECORD, FIELD_ID, FIELD_VALUE, MATCH_TYPE)
							select matched_results_seq.currval, 1, 1, account_name, null from subscriber where
							phone_number = matched_sub_row.matched_sub_phone_number and id = matched_sub_row.matched_subscriber_id and rownum < 2 ;
					end  if ;

					update ARCHIVED_MATCHED_DETAILS set field_value = matched_sub_row.matched_sub_phone_number where 
					...
					LOOP
						...
						Qry := 'update ARCHIVED_MATCHED_DETAILS set FIELD_VALUE = (select ' || db_field || ' from subscriber_v where ' ||
		...
		procedure Migrate(log_file_location IN VARCHAR2)
		IS
			CURSOR migrate_alarms_cursor IS SELECT * FROM archived_alarm@DU_NIKIRA A where 
				exists (select id from archived_alert@DU_NIKIRA AL where alarm_id = A.id
			...
			CURSOR migrate_alerts_cursor(in_alarm_id in number) IS SELECT * FROM archived_alert@DU_NIKIRA AL where alarm_id = in_alarm_id  
						and AL.event_instance_id in (select event_instance_id from event_rule_map EM where EM.event_instance_id = AL.event_instance_id) ;
		...
		BEGIN
			...
			fHandler := UTL_FILE.FOPEN(log_file_location, 'migrate_archived_alarms.log', 'w') ;
			...
			LOOP
				...
				SELECT JoinManyRowsIn1(R.key) into rule_ids_string FROM rules R where name IN (select E.description from event_instance@DU_NIKIRA E,
					archived_alert@DU_NIKIRA where alarm_id = alarm_row.ID and E.id = event_instance_id
					...
					select distinct (TAG_ID) TAGID FROM RULES_TAGS
								WHERE RULE_ID IN (SELECT ID
													...
													WHERE KEY in (SELECT KEY
																	...
																	FROM event_instance@DU_NIKIRA E, archived_alert@DU_NIKIRA A, EVENT_RULE_MAP EM where 
				...
				(
					select distinct(fraud_type_id) frdTypeID FROM FRAUD_TYPES_RULES F, RULES R 
										WHERE CompareCSV(R.ID, (SELECT JoinManyRowsIn1(max(ID)) from rules where 
											...
											KEY IN (select EM.rule_key FROM event_instance@DU_NIKIRA E, archived_alert@DU_NIKIRA A, EVENT_RULE_MAP EM where 
				...
				select name into case_name from cases where NAME = (select name from case@DU_NIKIRA where id = alarm_row.case_id) ;
				insert into archived_alarms(ID, NETWORK_ID, REFERENCE_ID, CASE_NAME, ALERT_COUNT, CREATED_DATE, MODIFIED_DATE, STATUS, USER_ID, OWNER_TYPE, SCORE,
				VALUE, CDR_COUNT, PENDING_TIME, REFERENCE_TYPE, REFERENCE_VALUE, IS_VISIBLE, RULE_IDS, DISPLAY_VALUE, FRAUD_TYPES, RULE_TAGS)
				values (alarm_row.ID, alarm_row.NETWORK_ID, alarm_row.SUBSCRIBER_ID, case_name, alarm_row.ALERT_COUNT, alarm_row.CREATED_DATE, 
				...
				insert into archived_alarm_comments(ID, COMMENT_DATE, USER_COMMENT, ALARM_ID, ACTION_ID, USER_ID) select alarm_comments_seq.nextval, 
						COMMENT_DATE, USER_COMMENT, ALARM_ID, ACTION_ID, USER_ID from archived_alarm_comment@DU_NIKIRA where alarm_id = alarm_row.ID ;

				exception when others then
					UTL_FILE.PUTF (fHandler, substr ('Error : insert into alarms ' || sqlcode || '-' || sqlerrm, 1, 250) || '\n') ;
				end ;

				insert into ARCHIVED_ALARMS_FRAUD_TYPES(alarm_id, fraud_type_id) select alarm_id, S.id 
					from ARCHIVED_ALARM_FRAUD_TYPE_MAP@DU_NIKIRA M, fraud_type@DU_NIKIRA F, fraud_types S
				...
				LOOP
					...
					insert into archived_alerts(ID, ALARM_ID, EVENT_INSTANCE_ID, VALUE, CDR_COUNT, REPEAT_COUNT, CDR_TIME, CREATED_DATE, MODIFIED_DATE,
					...
					insert into archived_alert_cdr(ALERT_ID, CDR_ID, CDR_VALUE, RECORD_CATEGORY, OWNER_ID) select distinct ALERT_ID, CDR_ID, CDR_VALUE,
						record_category_v, alarm_row.subscriber_id from archived_alert_cdr@DU_NIKIRA where alert_id = alert_row.ID ; 
